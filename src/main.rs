...
use crate::{board::*, cli::*, io::*, solver::*};

fn print_command(lines: &[&str], cli: &Shared) {
    let mut boards: Vec<Board> = Vec::new();
    let mut loaded = 0;
    for (j, line) in lines.iter().enumerate() {
        if cli.amount <= loaded {
            break;
        }
...
fn solve_command(lines: &Vec<&str>, cli: &Shared) {
    ...
    for (j, line) in lines.iter().enumerate() {
        ...
        let solved_board = match try_solve(&board, &BoardPoss::new()) {
            Some(b) => b,
            None => {
                println!("Something went wrong solving the Board.");
                continue;
            }
        };

        print!(
            "{:>len_digits$}/{:>len_digits$} solved\n{}",
            i,
            len,
            cursor::Up(1)
        );
        ...
        if cli.formated {
            ...
            output.push_str(
                get_encoded_board(&solved_board, &cli.delimeter, &cli.line_end_out).as_str(),
            );
        }
    }
    let default = PathBuf::from("./solutions.txt");
...
fn main() {
    ...
    let contents = match (
        &command_args.input_raw,
        &command_args.input_file_pos,
        &command_args.input_file_opt,
    ) {
        (None, Some(p), None) => {
            fs::read_to_string(p).expect("Should have been able to read the file")
        }
        (None, None, Some(t)) => {
            fs::read_to_string(t).expect("Should have been able to read the file")
        }
