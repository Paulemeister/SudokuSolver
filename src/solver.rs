...
fn check_squares(board: &Board,poss: &BoardPoss) -> BoardPoss{
    let mut new_poss = *poss;
    for i in 0..3{ // squares row
        for j in 0..3{ // squares collumn
            let mut square = PossField::new();
            // write existing numbers in square in PossField
            for l in 0..3{ // current square row
                for k in 0..3{ // current square collumn
...
fn replace_known(board: &mut Board, poss: &BoardPoss) -> Option<bool>{
    ...
    for (i,f) in poss.fields.iter().enumerate(){
        ...
        for j in 1..10 {
            if f.get(j) == Some(false){ amount+=1 }
        }
        if amount==8 { // one remaining? --> set remaining in board
            for j in 1..10 {
                if f.get(j).is_none(){
        ...
        else if amount==9 { // solved wrong or impossible
            return None
        }
    }
    Some(replaced)
...
pub fn try_solve(input_board:&Board,input_poss: &BoardPoss) -> Option<Board>{
    ...
    while has_none(&board){
        ...
        if !exchanged { // no trivial step left
